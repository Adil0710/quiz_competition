...
export async function GET(request: NextRequest, { params }: { params: { id: string } }) {
  try {
    ...
    const count = Math.max(1, Math.min(20, parseInt(countParam || '6', 10) || 6));
    ...
    const available = await Question.aggregate([
      { $match: { type, usedInCompetitions: { $ne: new mongoose.Types.ObjectId(competitionId) } } },
      { $sample: { size: count } }
    ]);
    ...
    await Question.updateMany(
      { _id: { $in: questionIds } },
      { $addToSet: { usedInCompetitions: new mongoose.Types.ObjectId(competitionId) } }
    );
